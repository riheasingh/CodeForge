...
const getStorageKey = (problemId, language) =>
  `leetlab:problem:${problemId}:code:${String(language || "").toUpperCase()}`;

const normalizeSubmissionLanguage = (language) => {
  const key = String(language || "").toUpperCase();
  if (key === "JAVASCRIPT" || key === "PYTHON" || key === "JAVA") return key;
  if (key === "JAVASCRIPT") return "JAVASCRIPT";
  return key;
};

const getLatestSubmissionCodeForLanguage = (submissions, language) => {
  const normalizedLang = String(language || "").toUpperCase();
  const filtered = (submissions || []).filter(
    (item) =>
      normalizeSubmissionLanguage(item.language) === normalizedLang &&
      typeof item.sourceCode === "string" &&
      item.sourceCode.trim().length > 0
  );
...
const ProblemPage = () => {
  ...
  useEffect(() => {
    if (problem) {
      ...
      const storageKey = getStorageKey(id, selectedLanguage);
      const savedCode = localStorage.getItem(storageKey);
      const latestSubmissionCode = getLatestSubmissionCodeForLanguage(
        submissions,
        selectedLanguage
      );
  ...
  }, [problem, selectedLanguage, submissions, id]);

  useEffect(() => {
    if (!id || !selectedLanguage || !problem) return;
    localStorage.setItem(getStorageKey(id, selectedLanguage), code);
  ...
  const handleSubmitSolution = async (e) => {
    ...
    try {
      ...
      // Keep user on the same page/tab after submit.
          ...
          <div className="card bg-base-100 shadow-xl">
            <div className="card-body p-0">
              <div className="tabs tabs-bordered">
                ...
                <Link to={`/problem/${id}/submissions`} className="tab gap-2">
                ...
                {latestSubmission && (
                  <div className="ml-auto flex items-center px-3">
                    <span
                      className={`badge ${
                        isLatestAccepted ? "badge-success" : "badge-error"
                      }`}
                    >
                      Latest: {latestSubmission.status}
                    </span>
                  </div>
                )}
              </div>
