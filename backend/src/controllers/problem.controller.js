...
export const createProblem = async (req, res) => {
  ...
  try {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (result.status.id === 13) {
          return res.status(503).json({
            error: `Execution engine internal error for language ${language}`,
            details:
              result.message ||
              result.stderr ||
              result.status?.description ||
              "Unknown execution engine internal error",
          });
        }
    ...
    return res.status(isExecutionEngineError ? 503 : 500).json({
      error: message,
    });
  }
};
